    ...
    fetch: async (url, opts) => {
        if (url.includes('/sync/')) {
            return {
                ...
                json: async () => ({
                    revision: 5,
                    profiles: { macbook: { settings: { config: { version: 1 } } }, mobile: { settings: { config: { version: 1 } } } }
                })
            };
        }
...
async function runAllTests() {
    ...
    runTest('videoScreenshot trigger: dùng cùng drag affordance với nút dịch phụ đề', () => {
        ...
        assert.match(
            triggerSource,
            /onMove:\s*\(\{\s*event,\s*deltaX,\s*deltaY,\s*origin\s*\}\)\s*=>\s*\{\s*floating\.stopFloatingEvent\(event\);/
        );
        assert.match(
            triggerSource,
    ...
    runTest('selectionCore: replaceSelectionSnapshot thay đổi giá trị input chuẩn xác', () => {
        ...
        mockInput.dispatchEvent = (evt) => {
            if (evt.type === 'input') dispatchedInput = true;
        };
