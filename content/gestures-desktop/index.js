    ...
    ext.features.gesturesDesktop = {
        ...
        init(context) {
            ...
            addListener(window, 'mousedown', (event) => {
                ...
                if (state.dblRight.lastEvent && now - state.dblRight.lastEvent.time < cfg.dblRightMs && dist(event.clientX, event.clientY, state.dblRight.lastEvent.x, state.dblRight.lastEvent.y) < TOLERANCE.move) {
                    event.preventDefault();
                    event.stopPropagation();
                    state.dblRight.lastEvent = null;
            ...
            addListener(window, 'wheel', (event) => {
                ...
                state.pager.acc += event.deltaX;
                clearTimeout(state.pager.timer);

                if (Math.abs(state.pager.acc) > cfg.pager.threshold) {
                    const dir = state.pager.acc > 0 ? 1 : -1;
                    state.pager.hops = dir !== state.pager.dir ? 1 : state.pager.hops + 1;
                    state.pager.dir = dir;
                    state.pager.acc = 0;
                    const maxHops = Math.max(1, Number(cfg.pager.hops) || 1);

                    showPagerIcon(state.pager.dir, state.pager.hops, maxHops);

                    state.pager.timer = setTimeout(() => {
                        hidePagerIcon();
                        const isMax = state.pager.hops >= maxHops;
                        goPage(state.pager.dir, Math.min(state.pager.hops, maxHops), isMax);
                        state.pager.acc = 0;
                        state.pager.dir = 0;
                        state.pager.hops = 0;
                    }, 240);
