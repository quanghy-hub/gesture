(() => {
    ...
    videoFloating.interactions.createTouchSwipeSeek = (targetFinder, noticeUI) => {
        ...
        const onTouchMove = (event) => {
            ...
            if (swipe.bridge)
                noticeUI.showSeekNotice({ duration: seekDuration, parentElement: videoFloating.core.utils.$('fvp-container') }, delta);
            ...
            if (vfConfig.realtimePreview && now - swipe.lastUpdate > vfConfig.throttle) {
                ...
                if (swipe.bridge) {
                    if (seekDuration > 0)
                        swipe.bridge.seekToRatio(videoFloating.core.utils.clamp(swipe.startTime + delta, 0, seekDuration) / seekDuration);
                } else {
        ...
        const onTouchEnd = (event) => {
            ...
            if (!swipe.cancelled && swipe.gesture === 'switch' && swipe.pendingSwitchDir) {
                ...
                if (swipe.bridge) {
                    ...
                    if (duration > 0)
                        swipe.bridge.seekToRatio(
                            videoFloating.core.utils.clamp(swipe.startTime + (swipe.lastDelta || 0), 0, duration) / duration
                        );
                } else if (swipe.video?.isConnected) {
                    swipe.video.currentTime = videoFloating.core.utils.clamp(
                        swipe.startTime + (swipe.lastDelta || 0),
                        0,
                        swipe.video.duration
                    );
                }
            } else if (!swipe.cancelled && swipe.bridge && swipe.lastDelta) {
                const duration = swipe.bridge.getDuration();
                if (duration > 0)
                    swipe.bridge.seekToRatio(videoFloating.core.utils.clamp(swipe.startTime + swipe.lastDelta, 0, duration) / duration);
            }
            resetSwipe();
        };
